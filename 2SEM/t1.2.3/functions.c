...
double SimpsonInt(double a, double b, int *n, double (*f)(double)) {
    double h, sum, x;
    if (*n % 2 != 0) *n = *n + 1; // Гарантируем четное количество интервалов
    h = (b - a) / *n;
    sum = f(a) + f(b);
    
    // Суммирование нечетных и четных точек
    for (int i = 1; i < *n; i++) {
        x = a + i * h;
        if (i % 2 == 0) {
            sum += 2 * f(x); // Четные точки
        } else {
            sum += 4 * f(x); // Нечетные точки
        }
    }
...
double Integral(double a, double b, double eps, int *n, int *err, double(*f)(double)) {
    ...
    *n = 2;
    prev = SimpsonInt(a, b, n, f);
    *n *= 2;
    current = SimpsonInt(a, b, n, f);
