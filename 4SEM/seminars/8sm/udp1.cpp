...
int main(void)
{
    ...
    hostent *hostinfo;
    hostinfo = gethostbyname("127.0.0.1");    // запрос DNS службы на получателя
    if ( hostinfo == nullptr ) {
    ...
    addr_other.sin_family = AF_INET;           // address family Internet
    addr_other.sin_addr = *(struct in_addr*) hostinfo->h_addr;  // IP адрес получателя
    addr_other.sin_port = htons(PORT_OTHER);   // порт преобразуется в сетевой byte order
    ...
    while (true) {
        ...
        // читаем с клавиатуры сообщение и отсылаем его
        printf(" > ");
